...
// go test -v -cover -count=1 -test.cpu=1 -run=^TestAESCTR$
func TestAESCTR(t *testing.T) {
	...
	for input, expect := range cases {
		crypted, err := aes.EncryptCTR(cryptox.PaddingPKCS7, testIV, []byte(input))
		if err != nil {
			t.Fatal(err)
		}
		...
		plain, err := aes.DecryptCTR(cryptox.UnPaddingPKCS7, testIV, crypted)
		if err != nil {
			t.Fatal(err)
		}
