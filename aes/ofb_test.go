...
// go test -v -cover -count=1 -test.cpu=1 -run=^TestAESOFB$
func TestAESOFB(t *testing.T) {
	...
	for input, expect := range cases {
		crypted, err := aes.EncryptOFB(cryptox.PaddingPKCS7, testIV, []byte(input))
		if err != nil {
			t.Fatal(err)
		}
		...
		plain, err := aes.DecryptOFB(cryptox.UnPaddingPKCS7, testIV, crypted)
		if err != nil {
			t.Fatal(err)
		}
